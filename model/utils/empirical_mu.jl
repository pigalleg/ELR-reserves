...
function construct_empirical_µ(gen_df,  loads, storage, reserve, energy_reserve, mip_gap = 1e-4)
    ...
    @variables(model, begin
        ERESUP[S, j in T, t in T; j <= t] >= 0
        ERESDN[S, j in T, t in T; j <= t] >= 0
    ...
    @constraint(model, EnergyResUpDisMin[s in S, j in T, t in T; j <= t],
        MAXERESUPDIS[s,t] >= ERESUPDIS[s,j,t]
    )
    ...
    @constraint(model, EpsilonsDnChMin[s in S, t in T],
        εDNCH[s,t] == sum(θDNCH[s,j]*ηch[s] for j in T if j<=t) - MAXERESDNCH[s,t]*ηch[s]
    )
...
#  Power constraints 
    ...
    # @constraint(model, EnergyResUpRequirement[j in T, t in T; j <= t],
    #     sum(ERESUP[i, j, t] for i in S) >= reserve[(reserve.i_hour .== j).&(reserve.t_hour .== t),:reserve_up_MW][1]
    # )

    # @constraint(model, EnerResDnRequirement[j in T, t in T; j <= t],
    #     sum(ERESDN[i, j, t] for i in S) >= reserve[(reserve.i_hour .== j).&(reserve.t_hour .== t),:reserve_down_MW][1]
    # )
    return model
end
